...
const ProductFeature: FC<ProductFeatureProps> = async ({ slice }) => {
  ...
  const fragrance =isFilled.contentRelationship(slice.primary.fragrance) ?
     await client.getByID<Content.FragranceDocument>(slice.primary.fragrance.id) : null;
  ...
  return (
    ...
    >
      <div className="grid grid-cols-1 items-center gap-8 lg:grid-cols-3 lg:grid-rows-[auto,auto]">
        <FadeIn start="top 70%" vars={{duration:1}} className="translate-y-16 opacity-0 lg:col-span-2 lg:row-span-2">
          <PrismicNextImage field={slice.primary.image} className="h-auto w-full object-cover" />
        </FadeIn>

        <FadeIn className="translate-y-16 space-y-6 bg-white/10 self-start p-10 opacity-0 lg:col-start-3 lg:row-start-1">
         ...
         <div className="text-base max-w-lg text-gray-300">
          ...
          <FadeIn vars={{duration:1, delay:1,}}  className="animate-in opacity-0 relative translate-y-16 self-end bg-white/10 will-change-transform">
